...
export default function BookingConfirmationPage() {
  const { vehicle, customerInfo, selectedAddons, resetBooking } = useBookingStore();
  const navigate = useNavigate();

  // Redirect if no vehicle (direct access to confirmation)
  useEffect(() => {
    if (!vehicle) {
      navigate('/');
    }
    ...
    <div className="bg-surface-container/30 min-h-screen pb-20">
      <div className="max-w-4xl mx-auto px-4 pt-12">
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="space-y-1">
                    <p className="font-bold text-on-surface">{customerInfo.name}</p>
                    <p className="text-sm text-secondary">{customerInfo.email}</p>
                    <p className="text-sm text-secondary">{customerInfo.phone}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 bg-surface-container/30 p-4 rounded-2xl border border-outline-variant/10">
                    ...
                    <div>
                      <p className="text-sm font-bold text-on-surface">Da Nang International Airport</p>
        ...
        <div className="mt-12 text-center">
          <p className="text-secondary text-sm">Need help with your booking? <Link to="/contact" className="text-primary font-bold hover:underline">Contact Support</Link></p>
        </div>
      </div>
    </div>
  );
