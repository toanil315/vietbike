...
export default function VehicleTable() {
  ...
  const [typeFilter, setTypeFilter] = useState<string>('Tất cả loại xe');
  const [locationFilter, setLocationFilter] = useState<string>('Tất cả địa điểm');
  ...
  const filteredVehicles = useMemo(() => {
    return VEHICLES.filter(v => {
      ...
      const matchesStatus = statusFilter === 'Tất cả trạng thái' || v.status === statusFilter.toLowerCase();
      const matchesType = typeFilter === 'Tất cả loại xe' || v.type === typeFilter.toLowerCase() || v.category === typeFilter.toLowerCase();
      const matchesLocation = locationFilter === 'Tất cả địa điểm' || (v.location && v.location.includes(locationFilter));
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary">Quản lý đội xe và tình trạng sẵn có của bạn.</p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-outline-variant/10 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container/30 border-b border-outline-variant/10">
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest">Thông tin xe</th>
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest">Loại xe</th>
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest">Biển số</th>
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest">Địa điểm</th>
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest text-right">Giá thuê ngày</th>
                <th className="px-6 py-4 text-[10px] font-bold text-secondary uppercase tracking-widest">Trạng thái</th>
        ...
        <div className="px-6 py-4 bg-surface-container/10 border-t border-outline-variant/10 flex items-center justify-between">
          <p className="text-sm text-secondary">
            Đang hiển thị <span className="font-bold text-on-surface">1-{filteredVehicles.length}</span> trên tổng số <span className="font-bold text-on-surface">{filteredVehicles.length}</span>
          </p>
